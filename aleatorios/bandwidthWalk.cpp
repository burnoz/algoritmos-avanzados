...
// Cambia aleatoriamente dos etiquetas de los nodos
map<int, int> chanegRandomTag(map<int, int>& nodos){
    vector<int> etiquetas;
    ...
    int idx1 = rand() % etiquetas.size();
    int idx2 = rand() % etiquetas.size();

    while(idx1 == idx2){
        idx2 = rand() % etiquetas.size();
    }

    swap(etiquetas[idx1], etiquetas[idx2]);
...
vector<int> gamble(map<int, int>& nodos, const vector<pair<int, int>>& aristas, int iterations){
    ...
    map<int, int> nodosSolution;

    // Shuffle inicial
    map<int, int> nuevasEtiquetas = newTags(nodos);
    bestCase = getMaxBandwidth(getBandwidths(nuevasEtiquetas, aristas));
    WorstCase = bestCase;
    nodosSolution = nuevasEtiquetas;

    for(int i = 0; i < iterations; i++){
        nuevasEtiquetas = chanegRandomTag(nodosSolution);
        int currentBandwidth = getMaxBandwidth(getBandwidths(nuevasEtiquetas, aristas));
        bandwidthResults.push_back(currentBandwidth);

        if(currentBandwidth < bestCase){
            bestCase = currentBandwidth;
            nodosSolution = nuevasEtiquetas;
        }

        if(currentBandwidth > WorstCase){
            WorstCase = currentBandwidth;
        }
    }

    avgBandwidth = getAvgBandwidth(bandwidthResults);

    // return {nodos, bestCase, WorstCase, avgBandwidth};
